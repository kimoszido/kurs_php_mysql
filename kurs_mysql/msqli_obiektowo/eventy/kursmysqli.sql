...
--
-- Host: localhost
-- Czas wygenerowania: 02 Sty 2012, 19:19
...
(1, 'testo@abc.pl', '', '', '', '', NULL, NULL, 0, '2011-12-30 13:11:47'),
(3, 'inny@abc.pl', 'abc', '', '', '', NULL, NULL, 0, '2011-12-30 13:11:59'),
(4, 'dome@abc.pl', 'ggggg', '', '', '', NULL, NULL, 0, '2011-12-30 13:12:04');
...
--
DROP VIEW IF EXISTS `widok_zamowien`;
...
--
DROP VIEW IF EXISTS `widok_zamowien_poszczegolnych_kursow`;
...
(1, 1, '309.00', '2011-12-31 11:33:30'),
(2, 1, '117.00', '2011-12-31 11:33:14'),
(3, 3, '34.00', '2011-12-31 11:32:43'),
(4, 4, '247.15', '2011-12-31 11:33:45');
...
-- Zdarzenia
--
DROP EVENT `update_energy`$$
CREATE DEFINER=`root`@`localhost` EVENT `update_energy` ON SCHEDULE AT '2012-01-26 19:20:17' ON COMPLETION PRESERVE ENABLE DO BEGIN
 UPDATE players SET energy = energy + 200 WHERE energy < max_energy;
END$$
